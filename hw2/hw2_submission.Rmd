...
```
NOTE:  Prof said this test is not really relevant here.  Wu-Hausman tests the similarty of coefficients between models, instead we can just discuss what we think to be true.  Also FWIW we do basically ignore every variable other than Y/Z/D etc

We are told that students are assigned to either small clases at random so we can assume Z is exogenious.  Comparing some of the demographic varibles using a t-test shows there is no significant different between the two groups among these variables.

TODO: run t-test between groups Z=0/1.
...
TODO
The effect on Z can be significant because it is possible that uncorelated variables become correlated when D is fixed = 1.  In other words there might be unobserved factors which caused D=1.  For example, maybe the the non-compliers have parents that requested a certain teacher or class size.  **something about colliding variables I need to read up on**
